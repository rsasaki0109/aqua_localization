...
def test_dry_run_prints_pipeline_commands(tmp_path):
    ...
    env.update(
        {
            ...
            "MBES_LOOP_SUBMAP_FINALIZE_DELAY_KEYFRAMES": "1",
    ...
    assert "MBES_LOOP_SUBMAP_FINALIZE_DELAY_KEYFRAMES=1" in proc.stdout
    ...
    assert "IMU_SONAR_ODOMETRY_TOPIC=" in proc.stdout
    assert (
        "IMU_PROFILE=/tmp/current_install/aqua_imu_loc/share/aqua_imu_loc/config/mbes_slam.yaml"
        in proc.stdout
    )
