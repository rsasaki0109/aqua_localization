...
def test_signature_match_rejects_descriptor_mismatch_when_configured(tmp_path):
    module = load_module()
    allowlist = tmp_path / "selected.csv"
    status = tmp_path / "status.csv"
    out = tmp_path / "audit.md"
    write_csv(
        allowlist,
        """
timestamp,current_keyframe_timestamp,candidate_keyframe_timestamp,current_id,candidate_id,fitness_score,correction_translation_m,correction_rotation_rad,descriptor_centroid_distance_m,descriptor_extent_ratio,descriptor_point_count_ratio
10.0,10.0,5.0,10,2,0.10,1.0,0.10,0.20,1.10,0.80
""",
    )
    write_csv(
        status,
...
10.1,10.2,5.1,110,20,1,accepted,0.11,1.2,0.12,2.00,1.12,0.78
""",
    )
    ...
    rc = module.main([
        ...
        "--signature-max-translation-delta-m", "0.5",
        "--signature-max-rotation-delta-rad", "0.05",
        "--signature-max-descriptor-centroid-delta-m", "0.5",
        "--signature-max-descriptor-extent-ratio-delta", "0.05",
        "--signature-max-descriptor-point-count-ratio-delta", "0.05",
    ])

    text = out.read_text(encoding="utf-8")
    assert rc == 2
    assert "- Descriptor signatures: 1" in text
    assert "- Accepted by signature: 0" in text
    assert "- Accepted outside allowlist: 1" in text
