...
GT for a fair shape comparison); right-hand plots show depth, DVL body-frame
velocity, and IMU acceleration. APE on this 15 s sequence is **0.43 m RMSE
over 5 296 samples** — the first public-data demo of the DVL fusion path.

![aqua_localization on Tank Dataset short_test (rerun.io)](docs/media/tank_dataset_rerun.png)

The `.rrd` recording (regenerated by
[`rerun_export.py`](aqua_localization/scripts/rerun_export.py)) opens
interactively in `rerun some.rrd` or via the rerun web viewer.

MBES-SLAM `beach_pond` — multibeam fans accumulated into world coordinates
using the bag's ROS 1 EKF reference and rendered in rerun.io. Each fan is
color-coded by depth, the cyan trajectory is the reference path that the
surface vessel followed, and the bathymetric structure of the lake bed
emerges directly from the accumulated registrations. The IMU dead-reckoning
trail is hidden in this default view because it drifts ~40 m RMSE on this
public bag — see [`datasets/mbes_slam_demo.md`](datasets/mbes_slam_demo.md)
for the full multibeam-degeneracy story and the registration tuning notes.

![aqua_localization on MBES-SLAM beach_pond (rerun.io)](docs/media/mbes_slam_rerun.png)

For the older 2D matplotlib view of the same registration run:
...
### Web replay (rerun.io / Lichtblick)

Two browser-friendly replay paths, both running on the same self-contained
demo bag (recipe in [`docs/foxglove/README.md`](docs/foxglove/README.md) — the
source sensors and `aqua_imu_loc` output topics get bundled into a single
`.mcap`).

- **rerun.io** (recommended for the headline view) — the
  [`rerun_export.py`](aqua_localization/scripts/rerun_export.py) script reads
  the bag, applies Umeyama SE(3) alignment to the estimate vs GT, and writes a
  ready-to-open `.rrd` with a curated 3D-plus-plots blueprint (the screenshot
  above is the headless `--screenshot-to` output). Open with
  `rerun some.rrd` or self-host the rerun web viewer.
- **Lichtblick** (Apache-2.0 fork of Foxglove Studio, no account required) —
  drop the same `.mcap` onto <https://lichtblick-suite.github.io/lichtblick/>
  ...
  Playwright driver loads the bag and layout headlessly for CI-style snapshots.
