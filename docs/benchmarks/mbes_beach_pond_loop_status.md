...
exports `/mbes_loop_closure/status`, and writes the benchmark row. The recorder
waits up to `RECORD_READY_TIMEOUT_S=75` for rosbag2 to subscribe to essential
output topics before starting playback. Playback then uses
`PLAY_START_DELAY_S=25` so rosbag2 can discover the source bag input
publishers before messages start flowing. Override `RECORD_READY_TOPICS` to
tune the output set. Keep both waits enabled for short replay windows and
normal-vs-selected comparisons; set them to `0` only for diagnostic runs where
startup coverage does not matter.
...
storage plugin. The runner creates a Humble-compatible sqlite3 source-window
bag with only the required input topics, then uses the explicit recording
storage and tuning overrides used for the measured row. Increase
`MBES_HUMBLE_WINDOW_S` when replaying longer than the default 180 s source
window:
...
| `plot_mbes_loop_audit.py` output | Plan-view PNG of the pose graph and accepted-loop audit priorities. The runner passes `--allow-empty` so strict selected replays with zero accepted loops still produce a plot artifact. |
