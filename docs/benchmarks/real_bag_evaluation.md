...
| MBES-SLAM `beach_pond` | measured | aqua_localization MBES frontend | dataset reference odometry, no-sonar fusion | translation APE RMSE, registration residual, loop accepted/rejected/no-candidate counts, descriptor gate pass rate | Audit the 17 accepted loops from the strict gap40 rotation/descriptor gate replay in RViz/rerun before using the row as accuracy evidence. |
