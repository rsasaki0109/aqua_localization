...
| MBES-SLAM `beach_pond` | measured | aqua_localization MBES frontend | dataset reference odometry, no-sonar fusion | translation APE RMSE, registration residual, loop accepted/rejected/no-candidate counts, descriptor gate pass rate | Use the strict gap40 geometry worksheet to finish RViz/rerun review: 16/17 accepted loops have keyframe positions, and the missing 1105 -> 1231 row must be resolved before using the row as accuracy evidence. |
