...
| Loop proposal quality | Candidate selection is temporal/spatial plus simple descriptor gates; selected-loop replay can now refine timestamp signatures with exported descriptor metrics, but there is still no offline retrieval/recall stage. | Sonar/bathymetry place recognition should propose loop candidates robustly under drift and low-resolution terrain. | Add bathymetric descriptor export and offline retrieval evaluation: recall@k against trusted loop/audit pairs, then use it before registration. |
...
3. **Measure the selected-loop replay.**
   ...
   survived and the pose graph was still worse than input odometry. The next
   step is descriptor-level loop retrieval/identity so more selected endpoints
   survive before registration, then a paired replay that shows whether the pose
