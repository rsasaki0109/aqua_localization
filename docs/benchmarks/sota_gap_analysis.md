...
3. **Measure the selected-loop replay.**
   ...
   when candidate endpoints are required to match. The next step is a stable
   loop identity or descriptor-level selector, then a paired replay that shows
   whether the pose graph improves against the dataset reference.
